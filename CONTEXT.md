...
**Remove** — the soft-delete action for a Class Board entity (Student, Lesson): sets `deleted_at` on the row rather than destroying it, hiding the entity from the UI without erasing its data. Used consistently in user-facing labels and confirmation dialogs (`RemoveStudentDialog`, `RemoveLessonDialog`) — see [ADR-0010](docs/adr/0010-soft-deleted-lessons-join-the-unique-index.md) for how removal interacts with uniqueness constraints.
